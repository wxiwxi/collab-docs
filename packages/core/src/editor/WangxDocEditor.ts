...
export type WangxDocEditorOptions<BSchema extends BlockSchema, ISchema extends InlineContentSchema, SSchema extends StyleSchema> = {
     ...
     */
    domAttributes: Partial<WangxDocDOMAttributes>
    ...
    /**
     * Use default WangxDoc font and reset the styles of <p> <li> <h1> elements etc., that are used in WangxDoc.
     *
     * @default true
     */
    defaultStyles: boolean

    schema: WangxDocSchema<BSchema, ISchema, SSchema>
...
const wangxDocTipTapOptions = {
    enableInputRules: true,
    enablePasteRules: true,
    enableCoreExtensions: false,
}

export class WangxDocEditor<
...
> {
    ...
    public readonly _tiptapEditor: WangxDocTipTapEditor & {
     ...
     */
    public readonly schema: WangxDocSchema<BSchema, ISchema, SSchema>
    ...
    >(options: Partial<WangxDocEditorOptions<BSchema, ISchema, SSchema>> = {}) {
        return new WangxDocEditor<BSchema, ISchema, SSchema>(options)
    }

    protected constructor(protected readonly options: Partial<WangxDocEditorOptions<any, any, any>>) {
        const anyOpts = options as any
        if (anyOpts.onEditorContentChange) {
            throw new Error(
                'onEditorContentChange initialization option is deprecated, use <WangxDocView onChange={...} />, the useEditorChange(...) hook, or editor.onChange(...)'
            )
        }

        if (anyOpts.onTextCursorPositionChange) {
            throw new Error(
                'onTextCursorPositionChange initialization option is deprecated, use <WangxDocView onSelectionChange={...} />, the useEditorSelectionChange(...) hook, or editor.onSelectionChange(...)'
            )
        }
        ...
        if (anyOpts.editable) {
            throw new Error(
                'editable initialization option is deprecated, use <WangxDocView editable={true/false} />, or alternatively editor.isEditable = true/false'
            )
        }
        ...
        const newOptions = {
            defaultStyles: true,
            schema: options.schema || WangxDocSchema.create(),
            _headless: false,
            ...options,
            placeholders: {
        ...
        const extensions = getWangxDocExtensions({
            editor: this,
        ...
        const wangxDocUIExtension = Extension.create({
            name: 'WangxDocUIExtension',
        ...
        extensions.push(wangxDocUIExtension)
        ...
        const tiptapOptions: WangxDocTipTapEditorOptions = {
            ...wangxDocTipTapOptions,
        ...
        if (!this.headless) {
            this._tiptapEditor = WangxDocTipTapEditor.create(tiptapOptions, this.schema.styleSchema) as WangxDocTipTapEditor & {
                contentComponent: any
            }
     ...
     *
     * @warning Not needed to call manually when using React, use WangxDocView to take care of mounting
     */
    ...
    /**
     * Inserts new blocks into the editor. If a block's `id` is undefined, WangxDoc generates one automatically. Throws an
     ...
     *
     * Use this method if you want to server-side render HTML (for example, a blog post that has been edited in WangxDoc)
     ...
     * `InlineNode` objects from any HTML inline elements, though not all element types are recognized. If WangxDoc
     ...
     * WangxDoc - children of blocks which aren't list items are un-nested and certain styles are removed.
     ...
     * Markdown syntax, though not all symbols are recognized. If WangxDoc doesn't recognize a symbol, it will parse it
     * as text.
     ...
     */
    public onChange(callback: (editor: WangxDocEditor<BSchema, ISchema, SSchema>) => void) {
        if (this.headless) {
     ...
     * @returns A function to remove the callback.
     */
    public onSelectionChange(callback: (editor: WangxDocEditor<BSchema, ISchema, SSchema>) => void) {
        if (this.headless) {
            return
        }
